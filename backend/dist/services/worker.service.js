...
import { checkAndIncrementHourlyLimit, checkSenderMinDelay, recordSenderSentTimestamp, getStartOfNextHourWindow, } from './ratelimit.service.js';
...
export function createEmailWorker() {
    ...
    console.log(`[Worker] Initializing BullMQ Worker on queue "${EMAIL_QUEUE_NAME}" with concurrency=${config.worker.concurrency}`);
    workerInstance = new Worker(EMAIL_QUEUE_NAME, async (job) => {
        const { emailId, userId, senderEmail, recipientEmail, subject, body, delayBetweenEmails, hourlyLimit } = job.data;
        ...
        if (!delayCheck.allowed) {
            console.log(`[Worker] Sender ${senderEmail} requires minimum ${minDelaySec}s spacing. Waiting ${delayCheck.waitTimeMs}ms...`);
        ...
        if (!rateCheck.allowed) {
            console.log(`[Worker] ⚠️ Sender ${senderEmail} exceeded hourly limit (${rateCheck.currentCount}/${effectiveLimit}). Rescheduling.`);
