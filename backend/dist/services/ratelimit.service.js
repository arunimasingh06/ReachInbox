...
return { 1, next_val }
`;
export function getHourWindowKey(senderEmail, date = new Date()) {
...
export function getStartOfNextHourWindow(date = new Date()) {
 ...
 */
export async function checkAndIncrementHourlyLimit(senderEmail, hourlyLimit) {
    const hourKey = getHourWindowKey(senderEmail);
    const ttlSeconds = 7200; // 2 hours retention
    // Execute atomic Lua script
    const result = (await redis.eval(ATOMIC_RATE_LIMIT_LUA, 1, hourKey, hourlyLimit.toString(), ttlSeconds.toString()));
 ...
 */
export async function checkSenderMinDelay(senderEmail, minDelaySeconds) {
 ...
 */
export async function recordSenderSentTimestamp(senderEmail) {
    const key = `sender:last_sent:${senderEmail}`;
    const now = Date.now();
