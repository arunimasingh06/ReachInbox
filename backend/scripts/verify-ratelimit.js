...
exports.runRateLimitVerification = runRateLimitVerification;
...
async function runRateLimitVerification() {
    ...
    const windowKey = (0, ratelimit_service_js_1.getHourWindowKey)(testSender);
    await redis_js_1.redis.del(windowKey);
    // Fire concurrent attempts simultaneously
    const results = await Promise.all(Array.from({ length: totalConcurrentAttempts }).map(() => (0, ratelimit_service_js_1.checkAndIncrementHourlyLimit)(testSender, hourlyCap)));
    const allowedCount = results.filter((r) => r.allowed).length;
    const deniedCount = results.filter((r) => !r.allowed).length;
    console.log(`Results: Allowed=${allowedCount}, Denied (Rescheduled)=${deniedCount}`);
    if (allowedCount !== hourlyCap) {
        throw new Error(`Rate limit verification FAILED: Expected exactly ${hourlyCap} allowed, got ${allowedCount}`);
    }
    if (deniedCount !== totalConcurrentAttempts - hourlyCap) {
        throw new Error(`Rate limit verification FAILED: Expected exactly ${totalConcurrentAttempts - hourlyCap} denied, got ${deniedCount}`);
    }
    // Verify next hour calculation
    const nextHour = (0, ratelimit_service_js_1.getStartOfNextHourWindow)();
    const now = new Date();
